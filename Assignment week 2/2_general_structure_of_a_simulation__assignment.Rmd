...
author: "Laura Hirt"
date: "`r format(Sys.time(), '29 02, 2024')`"
output:
  html_document:
    code_folding: hide
    highlight: haddock
...
The code in the below chunk is a copy of the final working simulation from "1_general_structure_of_a\_simulation.Rmd". Modify the code appropriately to create a new simulation that demonstrates the following:
...
analyse_data <- function(data) {
  ...
  student_t_test <- t.test(formula = score ~ condition, 
                       data = data,
                       var.equal = TRUE,
                       alternative = "two.sided")
  
  welch_t_test <- t.test(formula = score ~ condition, 
                       data = data,
                       var.equal = FALSE,
                       alternative = "two.sided")

  
  res <- tibble(p_student = student_t_test$p.value, 
                p_welch = welch_t_test$p.value)
...
simulation_summary <- simulation |>
  ...
  group_by(n_control,
           n_intervention) |>
  summarize(power_student = mean(p_student < .05),
            power_welch = mean(p_welch < .05), .groups = "drop")

# Table with proportion of significant p values
simulation_summary |>
  select(power_student, power_welch) %>%
  kable() |>
  add_header_above(header = c("Proportion of significant p values" = 2)) |>
  kable_classic(full_width = FALSE)

# join with other data 
simulation_enhanced <- simulation |>
  unnest(analysis_results) |>
  mutate(n_control = as.factor(n_control),
         n_intervention = as.factor(n_intervention)) |>
  left_join(simulation_summary, by = c("n_control", "n_intervention")) 

# Histograms

h_student <- ggplot(simulation_enhanced, aes(x = p_student)) +
  geom_histogram(breaks = seq(0, 1, by = 0.05), fill = "skyblue", color = "black") +
  coord_cartesian(ylim = c(0, 600)) +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "red", size = 1) +
  theme_minimal() +
  labs(title = "Histogram of p student", x = "p student", y = "Count")


h_welch <- ggplot(simulation_enhanced, aes(x = p_welch)) +
  geom_histogram(breaks = seq(0, 1, by = 0.05), fill = "salmon", color = "black") +
  geom_vline(xintercept = 0.05, linetype = "dashed", color = "red", size = 1) +
  coord_cartesian(ylim = c(0, 600)) +
  theme_minimal() +
  labs(title = "Histogram of p welch", x = "p welch", y = "Count")

# Arrange the plots vertically
grid.arrange(h_student, h_welch, ncol = 1)
```
